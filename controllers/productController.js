...
export const createProduct = async (req, res) => {
    try {
        ...
        const {
            ...
            category, subCategorySlug, subSubCategorySlug, brand, colors, attributes
        });
...
export const getLimitedStockedProducts = async (req, res) => {
    try {
        ...
        const limitedStockedProducts = await Product.find({ stock: { $lte: limitThreshold }, status: 'active' })
            .populate('category', 'name')
            .populate('subCategory', 'name')
            .populate('brand', 'name');

        await client.set(cacheKey, JSON.stringify(limitedStockedProducts), 'EX', 3600); // Cache for 1 hour
    ...
    try {
        ...
        const updatedProduct = await Product.findByIdAndUpdate(
            productId,
            {
                ...
                attributes: attributeObjs ? attributeObjs.map(attribute => attribute._id) : undefined,
                size,
                videoLink,
                userId,
                userType,
