...
export const createSubCategory = async (req, res) => {
  try {
    ...
    sendSuccessResponse(res, savedSubCategory, "Subcategory created successfully", 201);
  } catch (error) {
...
export const getSubCategoryById = async (req, res) => {
  try {
    ...
    if (cachedSubCategory) {
      ...
      return sendSuccessResponse(res, JSON.parse(cachedSubCategory), "Subcategory fetched successfully");
    }

    const subCategory = await SubCategory.findById(subCategoryId).populate("mainCategory", "name");
    if (!subCategory) {
...
export const getSubCategoryBySlug = async (req, res) => {
  try {
    ...
    const subCategory = await SubCategory.findOne({ slug }).populate("mainCategory", "name");
    if (!subCategory) {
      return sendErrorResponse(res, "Subcategory not found.", 404);
    }
...
export const updateSubCategoryById = async (req, res) => {
  try {
    ...
    sendSuccessResponse(res, updatedSubCategory, "Subcategory updated successfully");
  } catch (error) {
    sendErrorResponse(res, error.message);
  }
};
...
export const getSubCategoriesByMainCategorySlug = async (req, res) => {
  try {
    ...
    sendSuccessResponse(res, subCategories, "Subcategories fetched successfully");
  } catch (error) {
    sendErrorResponse(res, error.message);
  }
};
